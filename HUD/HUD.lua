...
--	mpMsgFontTex = "HUD/font_texturka",
	...
	DeadPlayerName = { "Dead", "Dead", "Dead", "Dead" },
	KillerPlayerName = { "Killer", "Killer", "Killer", "Killer" },
	AttackType = { 1, 1, 1, 1 },
	AttackTypeEnd = { 0, 0, 0, 0 },
	StatsPos = 0,
	FMessageStatusPos = { 0, 42, 82, 122 },
	LAmmosPlaySound = {
		{false, false},
		{false, false},
		{false, false},
...
--    self._matGrenadeIcon = MATERIAL.Create("HUD/raketa", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
	...
	CONSOLE.SetMPMsgFont( Cfg.HUD_ConsoleFont, self.mpMsgFontTex, Cfg.HUD_ConsoleFontSize )
...
function Hud:Tick(delta)
	...
	if not IsDedicatedServer() and Player then
		if Player.HasQuad and Cfg.HUD_PowerUp_Icons then
			...
			if Hud.HasQuadCountdown < 0 then
		...
		if Player.HasPentagram and Cfg.HUD_PowerUp_Icons then
			...
			if Hud.HasPentagramCountdown < 0 then
				Hud.HasPentagramCountdown = 30
			end
		elseif Hud.HasPentagramCountdown < 30 then
			Hud.HasPentagramCountdown = 30
		end
		if Player.HasWeaponModifier and Cfg.HUD_PowerUp_Icons then
			...
			if Hud.HasWeaponModifierCountdown < 0 then
				Hud.HasWeaponModifierCountdown = 30
			end
		elseif Hud.HasWeaponModifierCountdown < 30 then
			Hud.HasWeaponModifierCountdown = 30
		end
	end
end
...
function Hud:Render(delta)
    ...
    if Player then
      ...
      if not Hud.NoCrosshair then
        ...
        if type(w) == "number" and w <= 7 and w >= 1 and Player._CurWeaponIndex == w and Player._CurWeaponIndex == w then
