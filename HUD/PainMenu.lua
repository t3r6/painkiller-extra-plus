...
function PainMenu:AfterControlChange(name)
	...
	elseif self.currScreen == HUDConfig and name == "SetCrossPerW" then
		...
		local cweapon = { "Crosshair_PK", "Crosshair_SG", "Crosshair_GL", "Crosshair_RL", "Crosshair_LG", "Crosshair_SM", "Crosshair_BG", "Crosshair_All" }
		local tweapon = { "CrosshairTrans_PK", "CrosshairTrans_SG", "CrosshairTrans_GL", "CrosshairTrans_RL", "CrosshairTrans_LG", "CrosshairTrans_SM", "CrosshairTrans_BG", "CrosshairTrans_All" }
		local rweapon = { "CrosshairR_PK", "CrosshairR_SG", "CrosshairR_GL", "CrosshairR_RL", "CrosshairR_LG", "CrosshairR_SM", "CrosshairR_BG", "CrosshairR_All" }
		local gweapon = { "CrosshairG_PK", "CrosshairG_SG", "CrosshairG_GL", "CrosshairG_RL", "CrosshairG_LG", "CrosshairG_SM", "CrosshairG_BG", "CrosshairG_All" }
		local bweapon = { "CrosshairB_PK", "CrosshairB_SG", "CrosshairB_GL", "CrosshairB_RL", "CrosshairB_LG", "CrosshairB_SM", "CrosshairB_BG", "CrosshairB_All" }
		local sweapon = { "CrosshairSize_PK", "CrosshairSize_SG", "CrosshairSize_GL", "CrosshairSize_RL", "CrosshairSize_LG", "CrosshairSize_SM", "CrosshairSize_BG", "CrosshairSize_All" }
...
--	if not self.playerModel or not self.playerLight then return end
	...
	mx = mx - w / 1.28
	local weapons = { {"PKW_korpusShape", "PKW_HeadShape"}, {"ASG_bodyShape"}, {"stake"}, {"rl"}, {"pCylinderShape1", "pCylinderShape2", "polySurfaceShape450", "polySurfaceShape455"}, {"polySurfaceShape1391"}, {"polySurfaceShape431"} }
	...
	if rx < w / 1.067 and rx > w / 1.625 and ry > h / 12.8 and ry < h / 1.616 then
		...
		if INP.Key(Keys.MouseButtonLeft) == 1 then
			...
			if Cfg.PlayerModel > 7 then
				Cfg.PlayerModel = 1
			end
			if PainMenu.wtoplayerb > 1 or PainMenu.wtoplayera > 1 then
				PainMenu.wtoplayerb = 1
				PainMenu.wtoplayera = 1
			end
			PainMenu:ChangePlayerModel(MPModels[Cfg.PlayerModel])
