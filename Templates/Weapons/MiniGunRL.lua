...
function MiniGunRL:OnPrecache()
	...
	Cache:PrecacheParticleFX("HaingunHitWallNoSmoke") --OriOn_Modification
	Cache:PrecacheParticleFX("HaingunHitWallNoImpact") --OriOn_Modification
...
--OriOn_Modification=##########################################################################
	if not Cfg.NoMuzzleFlashFX then
...
function MiniGunRL:DrawHUD(delta)
    ...
    if not (INP.IsFireSwitched() or (not Game.SwitchFire[4] and Cfg.SwitchFire[4]) or (not Cfg.SwitchFire[4] and Game.SwitchFire[4])) then
		...
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*16)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.Grenades),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.Grenades)
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*50)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.MiniGun),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.MiniGun)
	else
		Hud:Quad(self._matAmmoMiniIcon,(1024-52*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*17)-Cfg.HUDSize*sizey)*h/768,Cfg.HUDSize,false)
		Hud:Quad(self._matAmmoRocketIcon,(1024-52*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*49)-Cfg.HUDSize*sizey)*h/768,Cfg.HUDSize,false)
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*16)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.MiniGun),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.MiniGun)
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*50)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.Grenades),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.Grenades)
	end
end
    ...
    if player then
        if not WEAPON_PREDICTION then
            if not Game.NoAmmoLoss then 
				player.Ammo.Grenades = player.Ammo.Grenades -1 
			end            
        end        
...
function MiniGunRL:OnUpdate()
    ...
    if self._ActionState == "AltFire" and self.ObjOwner.Ammo.MiniGun > 0 then
        if MPCfg.GameState == GameStates.Counting and Game._countTimer <= 3.99 then		--OriOn_Modification
			...
			--CONSOLE.AddMessage("test",R3D.RGB(0,255,0))
			return
        ...
        if self._ShotInterval <= 0 then
            self.shotFX = true
            if not Game.NoAmmoLoss then 
				self.ObjOwner.Ammo.MiniGun = self.ObjOwner.Ammo.MiniGun - 1 
				if Game:IsServer() then
					Game:AddToStats(self.ObjOwner.ClientID, AttackTypes.MiniGun, 0, 1, 0)
				end
			end
...
function MiniGunRL:HitWallSFX(entity,x,y,z,nx,ny,nz,fx,fy,fz)
	...
	elseif Cfg.NoSmoke and not Cfg.NoImpactFX then
		AddPFX("HaingunHitWallNoSmoke",0.25,Vector:New(px,py,pz),r)
	elseif not Cfg.NoSmoke and Cfg.NoImpactFX then
		AddPFX("HaingunHitWallNoImpact",0.25,Vector:New(px,py,pz),r)	--=end=OriOn_Modification
	end
