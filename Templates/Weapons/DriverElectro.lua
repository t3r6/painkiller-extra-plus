...
function DriverElectro:OnPrecache()
	...
	Cache:PrecacheParticleFX("electro_hit_wallNoSmoke") --OriOn_Modification
	Cache:PrecacheParticleFX("electro_hit_wallNoImpact") --OriOn_Modification
	Cache:PrecacheParticleFX("electro_hit")
	Cache:PrecacheParticleFX("electro_hitNoSmoke")		--OriOn_Modification
	Cache:PrecacheParticleFX("electro_hitNoInpact")		--OriOn_Modification
...
--OriOn_Modification=##########################################################################
	...
	if mode == 1 then -- normal
		if not Cfg.NoMuzzleFlashFX then
	        self._fxStart = BindFX(self._Entity,"electro_start1",0.04,"joint2",-0.06,0,0)
			--self._fxStart = BindFX(self._Entity,"electro_start1",0.04,"joint2",0.18,0.075,0.065)
    ...
    if mode == 2 then -- attack
		...
		if not Cfg.NoMuzzleFlashFX then 
	        --self._fxStart = BindFX(self._Entity,"electro_start2",0.04,"joint2",0.18,0.075,0.065)
	        --self._fxEnd = AddPFX("electro_end",0.8)
			self._fxStart = BindFX(self._Entity,"electro_start2",0.04,"joint2",-0.06,0,0)
	        self._fxEnd = AddPFX("electro_end",0.8)
			ENTITY.EnableGunPass(self._fxStart,true)
	        --for i=1, 15 do
	        --    self._particles[i] = AddPFX("electro",0.04)
    ...
    if self._ActionState == "AltFire" and self.ObjOwner.Ammo.Electro > 0 then
        ...
        if self._ShotInterval <= 0 then
            if not Game.NoAmmoLoss then 
				self.ObjOwner.Ammo.Electro = self.ObjOwner.Ammo.Electro - 1 
				if Game:IsServer() then	
					Game:AddToStats(self.ObjOwner.ClientID, AttackTypes.Electro, 0, 1, 0)
				end
			end
...
function DriverElectro:HitTest()
    ...
    if obj and obj.OnDamage then
		...
		elseif Cfg.NoSmoke and not Cfg.NoImpactFX then
			AddPFX("electro_hitNoSmoke",0.8,self.TipPoint)
		elseif not Cfg.NoSmoke and Cfg.NoImpactFX then
			AddPFX("electro_hitNoInpact",0.8,self.TipPoint)	--=end=OriOn_Modification
		end
...
function DriverElectro:FireSFX(pe,se,combo)
    ...
    if player then
        if not Game.NoAmmoLoss then 
			player.Ammo.Shurikens = player.Ammo.Shurikens - 1
		end
    ...
    if not self._lockedEntity then
        if b and e then
            ...
            if self._wallFryTimeOut <= 0 and not ENTITY.IsWater(e) then
				...
				elseif Cfg.NoSmoke and not Cfg.NoImpactFX then
					AddPFX("electro_hit_wallNoSmoke",0.5,self.TipPoint,r)
				elseif not Cfg.NoSmoke and Cfg.NoImpactFX then
					AddPFX("electro_hit_wallNoImpact",0.5,self.TipPoint,r)	--=end=OriOn_Modification
				end
