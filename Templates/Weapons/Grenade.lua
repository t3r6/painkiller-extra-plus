...
--OriON_Modification=#################################################
	if not Cfg.NoTrailGrenade then
...
function Grenade:OnPrecache()
	...
	Cache:PrecacheParticleFX("GrenadeNoSmoke")  --OriOn_Modification
	Cache:PrecacheParticleFX("GrenadeNoExplos") --OriOn_Modification
...
function Grenade:CL_GasFX(x,y,z)    
    ...
    for i=1, s.GrenadeWMGasAmount do
		...
		if not Cfg.NoExplosions then --OriOn_Modification
			obj:Apply()
			obj:Synchronize()
		end							 --OriOn_Modification
    end
end
...
function Grenade:Client_Explosion(entity,x,y,z)
	...
	elseif Cfg.NoSmoke and not Cfg.NoExplosions then
		AddPFX("GrenadeNoSmoke",0.4,Vector:New(x,y,z))
	elseif not Cfg.NoSmoke and Cfg.NoExplosions then
		AddPFX("GrenadeNoExplos",0.4,Vector:New(x,y,z))			--=end=OriOn_Modification
	end
    if Game.GMode == GModes.SingleGame then 
        -- physical parts
