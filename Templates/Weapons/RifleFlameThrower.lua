...
function RifleFlameThrower:OnPrecache()
	...
	Cache:PrecacheParticleFX("RifleHitWallNoSmoke")  	--OriOn_Modification
	Cache:PrecacheParticleFX("RifleHitWallNoImpact")	--OriOn_Modification
    ...
    if self._ActionState == "AltFire" and self.ObjOwner.Ammo.FlameThrower > 0 then
        ...
        if self._ShotInterval <= 0 then
            if not Game.NoAmmoLoss then 
				self.ObjOwner.Ammo.FlameThrower = self.ObjOwner.Ammo.FlameThrower - 1 
...
function RifleFlameThrower:HitWallSFX(entity,x,y,z,nx,ny,nz)
	...
	elseif Cfg.NoSmoke and not Cfg.NoImpactFX then
		AddPFX("RifleHitWallNoSmoke",0.2,Vector:New(x,y,z),r) 
	elseif not Cfg.NoSmoke and Cfg.NoImpactFX then
		AddPFX("RifleHitWallNoImpact",0.2,Vector:New(x,y,z),r) 	--=end=OriOn_Modification
	end
    ...
    for i = 1, n do
		...
		if Cfg.NoImpactFX then						--OriOn_Modification
			ENTITY.SetTimeToDie(ke,FRand(0,0))
		else
			ENTITY.SetTimeToDie(ke,FRand(1,2))		--OriOn_Modification
		end
    end
...
function RifleFlameThrower:FireSFX(pe)
    ...
    if player then
        if not Game.NoAmmoLoss then 
			player.Ammo.Rifle = player.Ammo.Rifle - 1 
		end
