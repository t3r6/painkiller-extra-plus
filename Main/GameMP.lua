...
function Game:NewPlayerRequest(clientID,name,model,team,state,spectator)
  ...
  if(spectator==nil)then spectator = 0 end
    ...
    Game:Server2ClientCommand(0, MPCfg.PCFWeapons and "enablepcfweaponsall" or "disenablepcfweaponsall")
    Game:SendRocketFix()
...
function Game:PlayerRespawnRequest(clientID)
    ...
    if player and not player._Entity then    
        ...
        if MPCfg.GameMode == "People Can Fly" and Cfg.PCFWeapons then -- required for compatibility with old clients
            for i,o in {"IShotgunFZ","IStakeGunGL","IDriverElectro","IRifleFlameThrower","IBoltGunHeater","IMiniGunRL"} do -- MiniGunRL takes precedence if last
...
function Game:BrightSkin(entity, enable, team)
    ...
    if entity and entity > 0 and not PMENU.Active() and teams and team == Cfg.Team and (not Player or entity ~= Player._Entity) and (Cfg.TeammateIcon == 1 or Cfg.TeammateIcon == 2) then
        ...
        0.5,0,0,0,ticon,color:Compose(),1,true)    
    ...
    if enable and MPCfg.AllowBrightskins then
        ...
        if (teams and Cfg.Team == team and not Cfg.FixedColors) or (Cfg.FixedColors and teams and team == 0) or (not teams and Player and entity == Player._Entity) then
            ...
            if Cfg.BrightskinTeam == "Purple" then color = Color:New(204,0,255) end
            if Cfg.BrightskinTeam == "Grey" then color = Color:New(153,153,153) end
            if Cfg.BrightskinTeam == "Brown" then color = Color:New(153,102,0) end
            ...
            if Cfg.BrightskinEnemy == "Purple" then color = Color:New(204,0,255) end
            if Cfg.BrightskinEnemy == "Grey" then color = Color:New(153,153,153) end
            if Cfg.BrightskinEnemy == "Brown" then color = Color:New(153,102,0) end
...
function Game:ConsoleClientMessage(clientID,txt,color)
    ...
    if Cfg.HUD_TeamChatOnly and ps.Team ~= Cfg.Team then return end
