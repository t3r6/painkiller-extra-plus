				...
				for i,o in Game.Players do
					if o.ClientID == botclientid then 
						...
						elseif(self.bot[botclientid].state == BotStates.Talking)then
							...
							if(math.random(6)>1 and false)then -- The condition is never true and will never be executed.
									Game.PlayerStats[botclientid].LastThingHeard = nil
									PLAYER.BotAction(Game.PlayerStats[botclientid]._Entity,Actions.SelectBestWeapon1,yaw,pitch,delta)
								return
							end
...
function Game:CheckBotCount()
    ...
    if Cfg.BotLimit and Cfg.BotLimit < Cfg.MaxPlayers then
      maxplayers = Cfg.BotLimit
		...
		if( Cfg.BotMinPlayers < maxplayers and (playercount > maxplayers-1 or playercount > Cfg.BotMinPlayers) ) and botcount > 0 then Console:Cmd_REPLACEBOT() return end 
		
		if( Cfg.BotMinPlayers == maxplayers and playercount < maxplayers-1 and playercount < Cfg.BotMinPlayers) then Console:Cmd_ADDBOT() return end 
		if( Cfg.BotMinPlayers == maxplayers and playercount > maxplayers or playercount > Cfg.BotMinPlayers) and botcount > 0 then Console:Cmd_REPLACEBOT() return end 		
		...
		if( Cfg.BotMinPlayers == maxplayers+1 and playercount > maxplayers-1 and speccount > 0) and botcount > 0 then Console:Cmd_REPLACEBOT() return end 		
		...
		if( Cfg.BotMinPlayers > maxplayers+1 and playercount > maxplayers ) and botcount > 0 then Console:Cmd_REPLACEBOT() return end 							
	end
end
--============================================================================
