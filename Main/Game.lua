...
function Game:PostRender(delta) -- do HUD'a
    ...
    if Cfg.HUD_HudStyle == 0 then
      wufontsizes = 26
      wufontcolortxt = {230,161,97}
      wufonttypo = "timesbd"
    else
      wufontsizes = 26
      wufontcolortxt = {200,200,200}
      wufonttypo = "Impact"
    end
    HUD.SetFont(wufonttypo, wufontsizes)
    HUD.PrintXY((w - HUD.GetTextWidth("Warm Up")) / 2 + 1, 601 * h / 768, "Warm Up", wufonttypo, 15, 15, 15, wufontsizes)
    HUD.PrintXY((w - HUD.GetTextWidth("Warm Up")) / 2, 600 * h / 768, "Warm Up", wufonttypo, wufontcolortxt[1], wufontcolortxt[2], wufontcolortxt[3], wufontsizes)
  end
...
function Game:LoadLevel(name)
    ...
    -- Compatibility with the original PK++ 1.3 servers
    -- start
